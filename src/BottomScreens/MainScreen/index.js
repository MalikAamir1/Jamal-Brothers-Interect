...
function MainScreen({navigation}) {
  ...
  console.log('AuthReducer on main screen: ', AuthReducer);
  ...
  useEffect(() => {
    LoadMoreRandomData();
    isDark?.isMatch ? getInterestData() : null;
    console.log('isDark?.isMatch: ', isDark?.isMatch);
  }, []);
  ...
  function LoadMoreRandomData() {
    setLoading(true);
    getRequestWithOutBody(`${BASE_URL}/getevents`, AuthReducer?.userData.userToken)
      .then(res => {
        let first = JSON.stringify(res);
        let second = JSON.parse(first);
        setEventData(second.data);
        console.log('eventData in LoadMoreRandomData', eventData);
        setLoading(false);
      })
      .catch(err => {
  ...
  function getInterestData() {
    ...
    getRequest(`${BASE_URL}/getinterest`)
      .then(res => {
        console.log('res from interest on main screen', res.data.interests);
        const newInterests = res.data.interests.map(data => data.interestName);
  
        if (AuthReducer?.userData.partnerProfile.partnerMergeInterest.length !== 0) {
          const intersection = newInterests.filter(element =>
            AuthReducer?.userData.partnerProfile.partnerMergeInterest.includes(element)
          );
          setInterestDataArray(intersection);
        } else {
          setInterestDataArray(newInterests);
        }
  ...
  const firstRenderItem = ({item}) => {
    return (
      <View
        ...
        <Pressable onPress={() => navigation.navigate('tinder', {item})}>
          <MainCard bgImg={Interest[0].bgImg} para={item} />
        </Pressable>
      </View>
    );
  };

  const ListFirstHeaderComponent = () => {
    return (
      <>
        <View>
          <Heading
  ...
  const ListSecondHeaderComponent = () => {
    return (
      <>
        {!!interestDataArray &&
          interestDataArray !== [] &&
            ...
            <>
              <View
                ...
                <FlatList
                  ...
                  ListHeaderComponent={<ListFirstHeaderComponent />}
                />
              </View>
            </>
          )}
  ...
  const ListThirdHeaderComponent = () => {
    return (
      <>
        <View style={{marginBottom: 10, marginTop: 30}}>
          <Heading
            ...
            {isDark?.isMatch ? (
              <View
                ...
                <FlatList
                  ...
                  ListHeaderComponent={<ListSecondHeaderComponent />}
                  showsVerticalScrollIndicator={false}
                />
              </View>
            ) : (
              <View
                ...
                <FlatList
                  ...
                  ListHeaderComponent={<ListThirdHeaderComponent />}
                  showsVerticalScrollIndicator={false}
                />
                {/* </View> */}
              </View>
